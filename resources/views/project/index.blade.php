    ...
    @forelse ($projects as $project)
        <a href="{{ route('project.show', ['id' => $project->id]) }}" class="group p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 flex items-center justify-between cursor-pointer">
            ...
            <div class="flex items-center gap-2">
                <button data-project-id="{{ $project->id }}" class="btn-delete-project opacity-0 invisible cursor-pointer transition-all duration-100 delay-500 group-hover:opacity-100 group-hover:visible">
                ...
                <button data-project-id="{{ $project->id }}" class="btn-edit-project opacity-0 invisible cursor-pointer transition-all duration-100 delay-500 group-hover:opacity-100 group-hover:visible">
