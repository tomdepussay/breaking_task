document.addEventListener('click', function (e){
    let button = e.target.closest('.btn-update-column');
    if(!button) return;

    let column_id = button.getAttribute('data-column-id');

    let form = document.getElementById('editColumnForm');
    ...
    fetch(`${columnRoutes.update}`, {
        ...
        .then(data => {
            ...
            document.querySelector(".modal-close[data-modal='updateColumn']").click();
        })
        .catch(error => {
            console.log(error);
        })
